...
import pytest
from strappy import Container, Provider, RegistrationMode, Scope
...
def test_add_second_provider_with_append_mode():
    ...
    container.add(second_mock_provider, mode=RegistrationMode.APPEND)
...
def test_add_second_provider_with_overwrite_mode():
    ...
    container.add(second_mock_provider, mode=RegistrationMode.OVERWRITE)
...
def test_registering_with_args_calls_add_provider_with_args():
    ...
    @container.register(
        ...
        mode=RegistrationMode.OVERWRITE,
    )
    class Service:
        def get_foo(self) -> str:
            return "foo"
    ...
    assert provider.instance is None
    assert container.add.call_args.kwargs["mode"] == RegistrationMode.OVERWRITE
